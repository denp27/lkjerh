...
# ==================== МОДУЛЬ ЮЗЕРБОТА (РЕАЛИСТИЧНЫЙ ПРОГРЕВ) ====================
...
class UserbotWorker:
    ...
    async def init_client(self, acc_name=None):
        ...
        # Имитируем реальное мобильное устройство для предотвращения слёта сессий
        ...
        if not await self.client.is_user_authorized():
            ...
            raise PermissionError(f"❌ Не удалось зайти по этому номеру ({phone_label}). Аккаунт не авторизован или сессия слетела.")
    ...
    async def run_single_warmup(self, acc_name, total_stages=6, progress_callback=None):
        ...
        try:
            ...
            for stage in range(1, total_stages + 1):
                logs = []
                # Живой человек выбирает случайное количество каналов за заход
                channels_to_interact = random.sample(WARMUP_CHANNELS, min(random.randint(2, 4), len(WARMUP_CHANNELS)))
                
                for ch in channels_to_interact:
                    try:
                        entity = await self.client.get_entity(ch)
                        
                        # 1. Естественная вероятность подписки (живой человек подписывается не всегда)
                        if random.random() < 0.4:
                            try:
                                await self.client(JoinChannelRequest(entity))
                                stats['subs'] += 1
                                stats['archive'] += 1
                                logs.append(f"✅ Осознанная подписка на @{ch}")
                                await asyncio.sleep(random.uniform(4.0, 9.0))
                            except Exception:
                                pass

                        # 2. Имитация чтения ленты с паузами
                        messages = await self.client.get_messages(entity, limit=random.randint(4, 12))
                        if messages:
                            await asyncio.sleep(random.uniform(2.0, 5.0))
                            await self.client(ReadHistoryRequest(peer=entity, max_id=messages[0].id))
                            stats['reads'] += 1
                            logs.append(f"👀 Прочитана лента @{ch}")
                            await asyncio.sleep(random.uniform(3.0, 7.0))

                            # 3. Выборочная реакция
                            ...
                            if random.random() < 0.5:
                                ...
                                try:
                                    ...
                                    stats['reactions'] += 1
                                    logs.append(f"👍 Поставлена реакция {emoji}")
                                    await asyncio.sleep(random.uniform(2.0, 4.0))
                                except Exception:
                                    pass

                            # 4. Редкое сохранение поста в Избранное
                            if random.random() < 0.2:
                                try:
                                    await self.client.forward_messages('me', target_msg)
                                    stats['saved'] += 1
                                    logs.append(f"📥 Сохранение полезного поста в Избранное")
                            ...
                            for msg in messages:
                                if getattr(msg, 'media', None) and hasattr(msg.media, 'poll'):
                                    try:
                                        poll = msg.media.poll
                                        if not poll.closed and poll.answers and random.random() < 0.6:
                                            ...
                                            stats['polls'] += 1
                                            logs.append(f"📊 Участие в голосовании (@{ch})")
                ...
                # 6. Живое сообщение в ЛС с эффектом «печатает...»
                other_accs = [a for a in accounts_db if a['name'] != acc_name]
                if other_accs and random.random() < 0.5:
                    peer_acc = random.choice(other_accs)
                    target_phone = peer_acc.get('phone') or peer_acc['name']
                    phrase = random.choice(PHRASES)

                    try:
                        async with self.client.action(peer_acc['session'], 'typing'):
                            await asyncio.sleep(random.uniform(2.0, 4.5))
                        await self.client.send_message(peer_acc['session'], phrase)
                        stats['pm'] += 1
                        logs.append(f"✉️ Живое сообщение ЛС ➔ {target_phone}: «{phrase}»")
                    except Exception:
                        pass

                delay_seconds = random.randint(2200, 4500)
                next_time_utc = (datetime.now(timezone.utc) + timedelta(seconds=delay_seconds)).strftime("%d.%m %H:%M UTC")

                log_text = "\n".join(logs) if logs else "⚡ Пауза в активности (имитация чтения)..."
                report = (
                    f"🔥 **Живой прогрев {phone_name} · этап {stage}/{total_stages}**\n\n"
                    f"{log_text}\n\n"
                    f"Статистика этапа:\n"
                    ...
                    f"**Этап {stage}/{total_stages} выполнен. Следующий шаг: {next_time_utc}**"
                )
            ...
            return f"✅ **Реалистичный прогрев для {phone_name} успешно завершен!**"
    ...
    async def broadcast(self, message_text: str, progress_callback=None) -> str:
        ...
        try:
            ...
            for index, username in enumerate(users):
                ...
                try:
                    async with self.client.action(username, 'typing'):
    ...
    try:
        if data == b"main_menu":
            ...
            auth_client = TelegramClient(
                acc_data['session'], API_ID, API_HASH, 
                proxy=telethon_proxy,
                device_model="Samsung Galaxy S23",
                system_version="Android 13.0"
            )
            ...
            buttons = [
                [Button.inline("🗑 Удалить прокси", b"proxy_menu")],
            ...
            await event.answer("🚀 Запуск живого прогрева аккаунта...")
            ...
            for acc in accounts_db:
                async def progress_cb(msg):
                    try:
                        ...
                        pass
                await event.respond(f"🚀 Старт живого прогрева для аккаунта: {acc['name']}")
...
async def text_handler(event):
    ...
    if state == 'awaiting_acc_name':
        ...
        await event.respond("⏳ Подключение к Telegram через прокси и запрос кода...")
        auth_client = TelegramClient(
            acc_data['session'], API_ID, API_HASH, 
            proxy=telethon_proxy,
            device_model="Samsung Galaxy S23",
        ...
        try:
            ...
            if not auth_client.is_connected():
                await event.respond("❌ Не удалось установить соединение с сервером Telegram через прокси.", buttons=get_main_keyboard())
                return
            ...
            await event.respond(
                "📲 Код успешно запрошен и отправлен!\n\n"
                "Введите полученный **код из Telegram** (также вы можете посмотреть его через кнопку «Получить код входа (Telegram 777000)» в меню аккаунта):"
            )
        except Exception as e:
