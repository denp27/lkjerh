...
class UserbotWorker:
    ...
    async def run_single_warmup(self, acc_name, total_stages=6, progress_callback=None):
        """Прогрев конкретного аккаунта по имени"""
        ...
        try:
            ...
            for stage in range(1, total_stages + 1):
                ...
                # Интервал между заходами (например, от 1 до 2 часов)
    ...
    async def broadcast(self, message_text: str, progress_callback=None) -> str:
        if not os.path.exists(PARSED_USERS_FILE):
            return "❌ Файл базы не найден. Сейчас выполните парсинг."
...
async def callback_handler(event):
    ...
    try:
        ...
        # --- МЕНЮ ПРОГРЕВА ---
            ...
            if not accounts_db:
                await event.answer("❌ Нет добавленных аккаунтов для прогрева!", alert=True)
                return
            await event.edit("🔥 **Меню прогрева аккаунтов (6 этапов):**", buttons=get_warmup_menu_keyboard())
            ...
            await event.edit(f"🔥 Запуск прогрева для `{acc_name}`...")
            async def progress(t):
                try: await event.edit(t)
                except Exception: pass
            res = await worker.run_single_warmup(acc_name, total_stages=6, progress_callback=progress)
            await event.edit(res, buttons=get_main_keyboard())

        elif data == b"warmup_all":
            await event.edit("🚀 Запуск массового прогрева всех аккаунтов...")
            for acc in accounts_db:
                p_name = acc.get('phone') or acc['name']
                await bot.send_message(event.chat_id, f"🔥 Старт прогрева для `{p_name}`...")
                
                async def progress_all(t):
                    try: await bot.send_message(event.chat_id, t)
                    except Exception: pass

                res = await worker.run_single_warmup(acc['name'], total_stages=6, progress_callback=None)
                await bot.send_message(event.chat_id, res)
            
            await bot.send_message(event.chat_id, "✅ **Массовый прогрев всех аккаунтов завершен!**", buttons=get_main_keyboard())
...
async def text_input_handler(event):
    ...
    if state == 'awaiting_acc_name':
        ...
        try:
            ...
            user_states.pop(event.sender_id, None)
            await msg.edit(f"❌ Не удалось зайти по этому номеру ({phone}). Ошибка: `{e}`", buttons=get_main_keyboard())
        ...
        try:
            ...
            user_states.pop(event.sender_id, None)
            auth_context.clear()
            await msg.edit(f"❌ Не удалось зайти по этому номеру ({phone}). Ошибка входа: `{e}`", buttons=get_main_keyboard())
