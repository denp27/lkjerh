...
# Структура: [{'id': int, 'type': socks.HTTP/SOCKS5, 'host': str, 'port': int, 'user': str, 'password': str}]
proxies_db = []

# Структура: [{'name': str, 'session': str, 'proxy_id': int, 'phone': str}]
accounts_db = []
...
def get_telethon_proxy(proxy_dict):
    """Преобразование словаря прокси в корректный формат Telethon / python-socks"""
    if not proxy_dict:
        return None
...
def get_accounts_keyboard():
    ...
    for acc in accounts_db:
        ...
        buttons.append([Button.inline(label, f"select_acc_{acc['name']}".encode())])
...
async def callback_handler(event):
    ...
    # --- МЕНЮ АККАУНТОВ ---
        ...
        await callback_handler(type('Event', (), {'sender_id': ADMIN_ID, 'data': b"acc_menu", 'edit': event.edit, 'answer': event.answer})())
        ...
        await event.edit(
            ...
            buttons=[
                [Button.inline("📱 По номеру телефона", f"auth_phone_{acc_name}".encode())],
                [Button.inline("🔲 По QR-коду", f"auth_qr_{acc_name}".encode())]
            ]
        )

    # --- МЕНЮ БАЗЫ ДАННЫХ ЮЗЕРОВ ---
    elif data == b"db_menu":
        total_count = 0
        if os.path.exists(PARSED_USERS_FILE):
            with open(PARSED_USERS_FILE, 'r', encoding='utf-8') as f:
        ...
        await callback_handler(type('Event', (), {'sender_id': ADMIN_ID, 'data': b"db_menu", 'edit': event.edit, 'answer': event.answer})())

    # --- МЕНЮ ПРОКСИ ---
    elif data == b"proxy_menu":
        await event.edit("🌐 **Список сохраненных Прокси:**", buttons=get_proxy_keyboard())
    ...
    # --- ДЕЙСТВИЯ ---
        ...
        await event.edit("🔥 Запуск прогрева...")
...
async def text_input_handler(event):
    ...
    if state == 'awaiting_acc_name':
        ...
        if not proxies_db:
            ...
            await event.respond(
                f"✅ Аккаунт `{acc_name}` создан!\n\nВыберите, к какому **прокси** привязать этот аккаунт:",
