...
class UserbotWorker:
    ...
    async def fetch_last_login_code(self, acc_name) -> str:
        """Получение последнего кода от официального аккаунта Telegram (777000) для конкретной сессии[cite: 1]"""
        try:
            acc = self.get_account_data(acc_name)
            if not acc:
...
async def callback_handler(event):
    ...
    try:
        if data == b"main_menu":
            ...
            p_name = acc.get('phone') if 'acc' in locals() and acc.get('phone') else "Не указан"
            info_text = (
