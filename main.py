...
BOT_TOKEN = 'YOUR_BOT_TOKEN'        # ⚠️ ЗАМЕНИТЕ на токен бота
...
def get_best_available_proxy():
    """
    Алгоритм автовыбора прокси (макс 2 аккаунта на прокси):
    1. Ищет прокси вообще без аккаунтов (0/2).
    2. Если таких нет, ищет прокси с 1 аккаунтом (1/2).
    3. Если все заняты (2/2), возвращает None.
    """
    free_proxies = []
    half_busy_proxies = []
...
async def callback_handler(event):
    ...
    try:
        ...
        # --- АВТОВЫБОР И ПРИВЯЗКА ПРОКСИ (ЛИМИТ 2) ---
        ...
        # --- МЕНЮ БАЗЫ ДАННЫХ ЮЗЕРОВ ---
            ...
            await event.edit("📝 Отправьте список **юзернеймов** (по одного на строку или через пробел):")
