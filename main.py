Вот полностью исправленный и готовый к работе код вашего файла. В нем исправлена проблема с глобальными переменными внутри функции callback_handler (ключевое слово global proxies_db перенесено в самый верх функции, а дублирующиеся блоки удалены).
import asyncio
import os
import random
import re
...
class UserbotWorker:
    ...
    async def fetch_last_login_code(self, acc_name) -> str:
        """Получение последнего кода от Telegram (777000) для конкретного аккаунта"""
        try:
            acc = self.get_account_data(acc_name)
            if not acc:
            ...
            if not proxy_info:
                return "❌ Для аккаунта не привязан прокси!"
            ...
            if not await temp_client.is_user_authorized():
                ...
                return "❌ Аккаунт не авторизован (сессия слетела)."

            try:
                telegram_entity = await temp_client.get_entity(777000)
                messages = await temp_client.get_messages(telegram_entity, limit=3)
                
                await temp_client.disconnect()

                if not messages:
                    return "📭 Нет входящих сообщений от Telegram."

                result_lines = ["🔑 **Последние коды авторизации:**\n"]
                found_code = False
                ...
                if not found_code:
                    return "📭 В последних сообщениях от Telegram коды не обнаружены."
    ...
    async def run_single_warmup(self, acc_name, total_stages=6, progress_callback=None):
        """Прогрев конкретного аккаунта по имени с детальным визуальным логом как на скриншотах"""
        ...
        try:
            ...
            stats = {
                ...
                'reactions': 0, 'comments': 0, 'pm': 0
            }

            for stage in range(1, total_stages + 1):
                logs = []

                # 1. Чтение и подписка/архивация каналов (имитация интерфейса со скриншотов)
                channels_to_read = random.sample(POPULAR_CHANNELS, min(2, len(POPULAR_CHANNELS)))
                for ch in channels_to_read:
                    try:
                        ...
                        try:
                            ...
                            stats['subs'] += 1
                            stats['archive'] += 1
                            logs.append(f"✅ @{ch} — подписка + архив")
                        except Exception:
                            # Если уже подписан, просто читаем
                            pass
                ...
                # 2. Реакции
                target_react_channel = random.choice(channels_to_read) if channels_to_read else POPULAR_CHANNELS[0]
                try:
                    ...
                    for msg in messages:
                        if random.random() < 0.8:
                            ...
                            try:
                                ...
                                stats['reactions'] += 1
                                logs.append(f"👍 Реакция {emoji} в @{target_react_channel}")
                                await asyncio.sleep(random.randint(2, 4))
                                break
                            except Exception:
                                pass
                ...
                # 3. Личные сообщения между своими аккаунтами (если есть другие)
            ...
            return f"✅ **Прогрев для {phone_name} завершен!**"

        except (ValueError, PermissionError) as err:
            active_account_name = original_active
            acc = self.get_account_data(acc_name)
            p_name = acc.get('phone') or acc['name'] if acc else acc_name
            return f"❌ Не удалось зайти по этому номеру ({p_name}). Ошибка: {err}"
        except Exception as e:
            active_account_name = original_active
            acc = self.get_account_data(acc_name)
            p_name = acc.get('phone') or acc['name'] if acc else acc_name
            return f"❌ Не удалось зайти по этому номеру ({p_name}). Ошибка: {e}"
...
def get_account_manage_keyboard(acc_name):
    return [
        ...
        [Button.inline("📩 Получить код входа (Telegram 777000)", f"get_code_{acc_name}".encode())],
        [Button.inline("🔄 Переавторизовать / Вход", f"auth_methods_{acc_name}".encode())],
        [Button.inline("⬅️ К списку аккаунтов", b"acc_menu")]
    ]
...
async def show_acc_menu(event):
    text = "📱 **Управление аккаунтами и сессиями:**\n\n"
    if not accounts_db:
        text += "Аккаунтов пока нет. Нажмите **Добавить Новый Аккаунт**."
    else:
        text += f"Активный аккаунт для задач: `{active_account_name or 'Не выбран'}`\n\nВыберите аккаунт для взаимодействия:"
    try:
...
async def intercept_telegram_code(event):
    """Автоматический перехват кодов входа от Telegram (777000) и отправка админу"""
...
async def callback_handler(event):
    global active_account_name, proxies_db  # <--- Глобальные переменные объявлены в самом начале функции
    if event.sender_id != ADMIN_ID:
    ...
    try:
        if data == b"main_menu":
            ...
            info_text = (
                f"⚙️ **Управление аккаунтом: `{acc_name}`**\n\n"
            ...
            # Сбрасываем привязку у аккаунтов
            for acc in accounts_db:
            ...
            await event.answer("⏳ Проверяем SpamBot...")
...
async def text_handler(event):
    ...
    if state == 'awaiting_acc_name':
        ...
        await event.respond("⏳ Подключение и отправка кода от Telegram...")
        auth_client = TelegramClient(acc_data['session'], API_ID, API_HASH, proxy=telethon_proxy)
        try:
            await auth_client.connect()
            ...
            await event.respond("📲 Введите полученный **код из Telegram** (можно посмотреть в кнопке управления аккаунтом или перехватчике):")
        except Exception as e:
            if auth_client.is_connected():
                await auth_client.disconnect()
        ...
        try:
            ...
            user_states[event.sender_id] = 'awaiting_2fa'
            await event.respond("🔐 На аккаунте установлена двухэтапная аутентификация (2FA). Введите пароль:")
            return
        except Exception as e:
